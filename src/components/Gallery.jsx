      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-14">
          ...
          <p className="section-subtitle max-w-xl mx-auto">
            A glimpse into our manufacturing facility, production operations and refractory products
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {PHOTOS.map((p, i) => (
            <div
              ...
              style={{ boxShadow: '0 2px 16px rgba(0,0,0,0.08)', border: '1px solid #e2e8f0' }}
            ...
            >
              ...
              >
                ...
                <p className="text-xs mt-0.5" style={{ color: '#fbbf24' }}>{p.sub}</p>
              </div>
              {/* Amber corner accent */}
              <div
          ...
          >
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5"
                ...
                <p className="text-sm" style={{ color: '#fbbf24' }}>{PHOTOS[lightbox].sub}</p>
              </div>
              {/* Close */}
              <button
                className="absolute top-4 right-4 w-9 h-9 rounded-full flex items-center justify-center font-bold text-white transition-colors"
              ...
              {lightbox > 0 && (
                <button
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full flex items-center justify-center text-white font-bold transition-colors"
                  style={{ background: 'rgba(0,0,0,0.6)' }}
              ...
              {lightbox < PHOTOS.length - 1 && (
                <button
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full flex items-center justify-center text-white font-bold transition-colors"
                  style={{ background: 'rgba(0,0,0,0.6)' }}
                  onClick={e => { e.stopPropagation(); setLightbox(lightbox + 1) }}
                  onMouseEnter={e => e.currentTarget.style.background='rgba(217,119,6,0.9)'}
                  onMouseLeave={e => e.currentTarget.style.background='rgba(0,0,0,0.6)'}
