...
export default function Navbar() {
  ...
  return (
    <header
      ...
      style={{ background: bg, backdropFilter: 'blur(12px)', boxShadow: scrolled ? '0 2px 24px rgba(0,0,0,0.4)' : 'none' }}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <a href="#home" onClick={close} className="flex items-center gap-3">
            ...
            <div>
              <div className="text-white font-bold text-sm leading-tight tracking-widest">CHAMPION</div>
              <div className="text-xs leading-tight font-medium" style={{ color: '#fbbf24' }}>MAGNESITE LLP</div>
            </div>
          </a>
          ...
          <nav className="hidden lg:flex items-center gap-0.5">
            {NAV.map(l => (
              <a key={l.href} href={l.href}
                ...
                style={{ color: '#cbd5e1' }}
                onMouseEnter={e => { e.target.style.color='#fbbf24'; e.target.style.background='rgba(255,255,255,0.06)' }}
                onMouseLeave={e => { e.target.style.color='#cbd5e1'; e.target.style.background='transparent' }}>
                {l.label}
              </a>
            ))}
            <a href="tel:9229273143"
              className="ml-3 px-5 py-2.5 rounded-lg text-sm font-bold transition-all duration-200"
          ...
          {/* Mobile toggle */}
          <button className="lg:hidden p-2 text-white rounded-lg" onClick={() => setOpen(!open)}>
            {open
        ...
        {/* Mobile menu */}
        {open && (
          <nav className="lg:hidden pb-5 border-t" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
